  ...
  return (
    <HoverCard>
      <HoverCardTrigger asChild>
        <Card className="group cursor-pointer hover:border-primary/40 hover:shadow-md transition-all duration-300 bg-card/80 h-full flex flex-col">
          <CardHeader className="pb-3">
            <div className="flex items-start justify-between gap-3 mb-3">
              <span className="text-2xl">{guide.icon}</span>
              <Badge variant="outline" className={`text-[10px] font-mono shrink-0 ${levelColor[guide.level]}`}>{guide.level}</Badge>
            </div>
        ...
        <div className="flex gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-3">
              <Badge variant="outline" className={`text-[10px] font-mono ${levelColor[guide.level]}`}>{guide.level}</Badge>
...
export default function GuidesPage() {
  ...
  return (
    <PageShell badge="Resources" title="Guides & Tutorials" subtitle="Practical, step-by-step tutorials to help you get the most out of TriVisionX.">
      ...
      <Section title="Can't find what you need?" className="text-center border-t mt-16 pt-12">
        <p className="text-muted-foreground mb-6">Request a guide or ask questions in the community.</p>
        <Link href="/community" className="inline-flex items-center gap-2 text-sm font-semibold text-primary hover:underline">
          <BookOpen className="w-4 h-4" /> Visit the Community →
        </Link>
      </Section>
    </PageShell>
  );
